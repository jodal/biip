...
GTIN, short for Global Trade Item Number,
is the data hiding in the UPC or EAN-13 barcodes
found on nearly all products you can by in a store.

GTINs are either 8, 12, 13, or 14 characters long,
and named either GTIN-8, GTIN-12, GTIN-13, or GTIN-14.
Let's use the GTIN-13 ``5901234123457`` as an example::
...
If you're looking up the trade item in a database,
you probably want to normalize the GTIN to a GTIN-14 first::
...
By storing all GTINs as GTIN-14, and
converting all incoming GTINs to GTIN-14 before comparing with the database,
we can make product lookup an exact science.
...
The lack of error messages from the SSCC and GS1 Message parsers reflect this::
...
As you can see, the number is detected to be an RCN,
...
- An *Application Identifier* (AI) is a number specifying a data field's format and use.
- An AI prefix, together with its data field, is called an AI *Element String*.
...
we might get the data ``00376130321109103420``:
...
From the result, we can see that the data is a Message that contains a single Element String.
...
containing multiple trade units, we might get
...
How do then know where the Element Strings ends,
...
If we use an alternative separator character,
we need to tell Biip what character to expect::
